...
export class CalendarEventViewModel {
	...
	setAllDay(newAllDay: boolean): void {
		...
		if (newAllDay) {
			...
			const startTime = this.startTime ?? new Time(0, 0)
	...
	/**
	 * add an exclusion to the repeat rule of this event, which will cause the event instance that starts at the excluded time to not be rendered or
	 * fire alarms.
	 *
	 * the list of exclusions is maintained sorted from earliest to latest.
	 */
	async excludeOccurrence(): Promise<void> {
...
export function areExcludedDatesEqual(e1: ReadonlyArray<DateWrapper>, e2: ReadonlyArray<DateWrapper>): boolean {
	...
	return e1.find(({ date }, i) => e2[i].date.getTime() !== date.getTime()) == null
}
