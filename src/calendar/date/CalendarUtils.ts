...
export function calendarEventHasMoreThanOneOccurrencesLeft(event: CalendarEvent): boolean {
	...
	if (endType === EndType.Never) {
		...
		let i = 0
		const notExcludedOccurrences: Array<number> = []
		iterateOccurrencesWhileTrue(event, getTimeZone(), (startTime) => {
			if (notExcludedOccurrences.length > 1) {
				return false
			}
			...
			if (startTimestamp !== excludedTimestamps[i]) {
				notExcludedOccurrences.push(startTimestamp)
			}
			return true
		})

		return notExcludedOccurrences.length > 1
	}
}
...
function isExcludedDate(currentDate: Date, excludedDates: ReadonlyArray<DateWrapper> = []): boolean {
	return excludedDates.find((dw) => dw.date.getTime() === currentDate.getTime()) != null
}
...
export function findNextAlarmOccurrence(
	...
	while (endType !== EndType.Count || occurrenceNumber < endValue) {
		...
		if (exclusions.find((d) => d.getTime() === occurrenceDate.getTime()) == null) {
