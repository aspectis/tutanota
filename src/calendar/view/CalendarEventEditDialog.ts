	...
	return [
		renderTwoColumnsIfFits(
			[
				...
				m(".flex-grow.pr-s", renderRepeatPeriod(viewModel)), // Repeat interval: every day, every second day etc
				m(".flex-grow.pl-s" + (viewModel.repeat ? "" : ".hidden"), renderRepeatInterval(viewModel, intervalValues)),
			],
			viewModel.repeat
				? [
